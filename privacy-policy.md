...
**Actions the AI can propose:** In addition to answering your questions in plain English, the AI Chat feature can propose actions on your data, including: creating new transactions, editing existing transactions, deleting transactions, creating investment transactions, and creating or modifying budgets. These proposals are not executed automatically. Each proposed action is queued as a pending action and surfaced to you for explicit confirmation before any change is made to your data. You may approve, modify, or reject each proposal individually. No action proposed by the AI takes effect without your direct approval.
...
- *Transactions:* date, type (expense, income, refund, transfer, or currency exchange), category and subcategory, note field (which may include merchant names you have entered), amount, currency, home-currency equivalent, source and destination account names, and recurring or instalment tags.
...
- *Context:* your home currency, today's date, current exchange rates for the currencies in use across your accounts and portfolio, and the list of category and subcategory names you have configured.
...
**Pseudonymity of requests to Google:** Our server does not forward your device identifier, your originating IP address, or any Apple-specific identifier to Google. The request payload contains financial data but no name, email, or Apple ID. From Google's perspective, all AI Chat requests from the App come from our developer account and our server's IP address, without explicit identifiers tying each request to a specific individual. However, the content of a request (such as transaction notes, account names, and merchant names you have entered) may itself indirectly identify a person if it contains personal references. We have no mechanism to retrieve a specific user's requests from Google's logs.

**Rate limiting:** To enforce daily usage limits, our server maintains an in-memory count of requests keyed to a device-scoped identifier transmitted by the App, used solely for the purpose of enforcing rate limits. This identifier is not used for any other purpose, is not linked to your name, email, or Apple ID, and is not shared with any third party. Rate-limit data is held in volatile memory only and is not persisted to disk or any database. The daily limit is 5 requests for Free and Pro users and 200 requests for Premium users.

**International data transfers:** Our server infrastructure is hosted by Railway in Singapore. Google's Gemini API processes requests in Google's global data centres, which may be located outside Singapore. By using the AI Chat feature, you acknowledge that your data may be transmitted to and processed in jurisdictions outside Singapore as part of Google's processing. We rely on the contractual protections provided by Google to maintain a comparable standard of data protection to that required under Singapore's Personal Data Protection Act.
